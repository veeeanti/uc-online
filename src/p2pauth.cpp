...
#include <algorithm>
//-----------------------------------------------------------------------------
// Purpose: Send auth ticket to connection
//-----------------------------------------------------------------------------
bool SendAuthTicketToConnection( CSteamID steamIDFrom, HSteamNetConnection hConnectionTo, uint32 cubTicket, uint8 *pubTicket )
{
    ...
    msg.SetToken(pubTicket, cubTicket);
    
    int64 nIgnoreMessageID;
    return SteamNetworkingSockets()->SendMessageToConnection(hConnectionTo, &msg, sizeof(msg), k_nSteamNetworkingSend_Reliable, &nIgnoreMessageID) == k_EResultOK;
}
